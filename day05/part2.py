...
class Validator:
    ...
    def reorder(self, pages: list[int]) -> None:
        for i in range(len(pages) - 1):
            for j in range(i + 1, len(pages)):
                if not self.has_rule(pages[i], pages[j]):
    ...
    def start(self) -> int:
        ...
        for line in self.second.splitlines():
            ...
            if not self.is_valid(pages):
                # print(pages)
                self.reorder(pages)  # modifies in-place
                # print("# after reordering:", pages)
